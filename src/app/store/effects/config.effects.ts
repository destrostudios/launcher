...
export class ConfigEffects {
  ...
  loadLatestConfig = createEffect(() => this.actions.pipe(
    ofType(ConfigActions.loadClientConfigs),
    switchMap(() => this.configHttpService.getClientConfigs().pipe(
      map(configs => ConfigActions.loadClientConfigsSuccessful({ configs })),
      catchError(error => of(ConfigActions.loadClientConfigsError({ error })))
    ))
  ));
}
