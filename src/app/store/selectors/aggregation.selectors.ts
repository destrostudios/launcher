...
export const isLoginLoading = createSelector(
  ...
  (saltClientLoading, sessionIdLoading, currentUser, clientConfigsLoading, latestNewsLoading, appsLoading) => {
    return Boolean(saltClientLoading || sessionIdLoading || (currentUser && (clientConfigsLoading || latestNewsLoading || appsLoading)));
  },
);

export const getFeaturedApp = createSelector(
  getFeaturedAppId, getApps,
  (featuredAppId, apps) => {
...
export const hasOwnedApps = createSelector(
  getOwnedApps, ownedApps => ownedApps && ownedApps.length > 0
);

export const getDisplayedLibraryApps = createSelector(
  getOwnedApps, getLibrarySearchText,
...
export const isSelectedAppOwned_Store = createSelector(
  getSelectedApp_Store, getOwnedAppIds,
