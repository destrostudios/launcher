...
export function searchApps(apps: App[], searchText: string): App[] {
  ...
  return apps.filter(
    (app) => app.name.toLowerCase().indexOf(lowercaseSearchText) !== -1,
  );
}

export function updateLocalApps(
  localApps: LocalApp[],
  appId: number,
  updatedLocalApp: (LocalApp) => LocalApp,
): LocalApp[] {
...
export function getOutdatedAppFiles(localApp: LocalApp): AppFile[] {
  return localApp.outdatedFileIds.map((appFileId) => {
    return localApp.files.data.files.find(
      (appFile) => appFile.id === appFileId,
    );
  });
}
